...
public class ContainerListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<ContainerSummary>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, ContainerListOkException {
			throw new ContainerListOkException(responseValue.get());
		}
	...
	public ContainerListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			ContainerListOkException, BadRequestErrorResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher
					.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, ContainerSummary.class, httpResponse));
			break;
		}
		case 400: {
	...
	public List<ContainerSummary> unwrap() throws IOException, JsonParseException, JsonMappingException,
			BadRequestErrorResponseException, InternalServerErrorErrorResponseException {
		return match(new Matcher<List<ContainerSummary>>() {
			@Override
			public List<ContainerSummary> caseOk(ResponseValue<List<ContainerSummary>> responseValue)
