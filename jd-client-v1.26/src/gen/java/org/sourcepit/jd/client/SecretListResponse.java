...
public class SecretListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Secret>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, SecretListOkException {
			throw new SecretListOkException(responseValue.get());
		}
	...
	public SecretListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			SecretListOkException, InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Secret.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Secret> unwrap() throws IOException, JsonParseException, JsonMappingException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<List<Secret>>() {
			@Override
			public List<Secret> caseOk(ResponseValue<List<Secret>> responseValue)
