...
public class TaskListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Task>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, TaskListOkException {
			throw new TaskListOkException(responseValue.get());
		}
	...
	public TaskListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			TaskListOkException, InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Task.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Task> unwrap() throws IOException, JsonParseException, JsonMappingException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<List<Task>>() {
			@Override
			public List<Task> caseOk(ResponseValue<List<Task>> responseValue)
