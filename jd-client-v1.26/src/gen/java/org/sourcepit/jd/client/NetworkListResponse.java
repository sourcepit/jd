...
public class NetworkListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Network>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, NetworkListOkException {
			throw new NetworkListOkException(responseValue.get());
		}
	...
	public NetworkListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			NetworkListOkException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Network.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Network> unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<List<Network>>() {
			@Override
			public List<Network> caseOk(ResponseValue<List<Network>> responseValue)
