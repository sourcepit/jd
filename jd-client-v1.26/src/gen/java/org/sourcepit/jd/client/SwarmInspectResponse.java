...
public class SwarmInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<SwarmInspectOkResponse> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSwarmInspectOkResponseException {
			throw new OkSwarmInspectOkResponseException(responseValue.get());
		}
	...
	public SwarmInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSwarmInspectOkResponseException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, SwarmInspectOkResponse.class, httpResponse));
			break;
		}
		case 404: {
	...
	public SwarmInspectOkResponse unwrap()
			throws IOException, JsonParseException, JsonMappingException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<SwarmInspectOkResponse>() {
			@Override
			public SwarmInspectOkResponse caseOk(ResponseValue<SwarmInspectOkResponse> responseValue)
