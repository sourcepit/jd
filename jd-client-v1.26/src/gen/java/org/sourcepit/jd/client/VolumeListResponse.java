...
public class VolumeListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<VolumeListOkResponse> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkVolumeListOkResponseException {
			throw new OkVolumeListOkResponseException(responseValue.get());
		}
	...
	public VolumeListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkVolumeListOkResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, VolumeListOkResponse.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public VolumeListOkResponse unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<VolumeListOkResponse>() {
			@Override
			public VolumeListOkResponse caseOk(ResponseValue<VolumeListOkResponse> responseValue)
