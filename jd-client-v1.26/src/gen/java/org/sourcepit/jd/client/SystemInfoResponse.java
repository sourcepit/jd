...
public class SystemInfoResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<SystemInfoOkResponse> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSystemInfoOkResponseException {
			throw new OkSystemInfoOkResponseException(responseValue.get());
		}
	...
	public SystemInfoResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSystemInfoOkResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, SystemInfoOkResponse.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public SystemInfoOkResponse unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<SystemInfoOkResponse>() {
			@Override
			public SystemInfoOkResponse caseOk(ResponseValue<SystemInfoOkResponse> responseValue)
