...
public class ContainerChangesResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<ContainerChangesOkResponse>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, ContainerChangesOkException {
			throw new ContainerChangesOkException(responseValue.get());
		}
	...
	public ContainerChangesResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			ContainerChangesOkException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(
					new ResponseValue<>(objectMapper, ArrayList.class, ContainerChangesOkResponse.class, httpResponse));
			break;
		}
		case 404: {
	...
	public List<ContainerChangesOkResponse> unwrap() throws IOException, JsonParseException, JsonMappingException,
			NotFoundErrorResponseException, InternalServerErrorErrorResponseException {
		return match(new Matcher<List<ContainerChangesOkResponse>>() {
			@Override
			public List<ContainerChangesOkResponse> caseOk(
					ResponseValue<List<ContainerChangesOkResponse>> responseValue)
