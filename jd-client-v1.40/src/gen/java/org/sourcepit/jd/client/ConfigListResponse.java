...
public class ConfigListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Config>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, ConfigListOkException {
			throw new ConfigListOkException(responseValue.get());
		}
	...
	public ConfigListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			ConfigListOkException, InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Config.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Config> unwrap() throws IOException, JsonParseException, JsonMappingException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<List<Config>>() {
			@Override
			public List<Config> caseOk(ResponseValue<List<Config>> responseValue)
