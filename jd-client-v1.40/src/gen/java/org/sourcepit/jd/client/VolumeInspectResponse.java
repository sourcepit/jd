...
public class VolumeInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<Volume> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkVolumeException {
			throw new OkVolumeException(responseValue.get());
		}
	...
	public VolumeInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkVolumeException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, Volume.class, httpResponse));
			break;
		}
		case 404: {
	...
	public Volume unwrap() throws IOException, JsonParseException, JsonMappingException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException {
		return match(new Matcher<Volume>() {
			@Override
			public Volume caseOk(ResponseValue<Volume> responseValue)
