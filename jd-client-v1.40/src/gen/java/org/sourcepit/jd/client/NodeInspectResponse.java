...
public class NodeInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<Node> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkNodeException {
			throw new OkNodeException(responseValue.get());
		}
	...
	public NodeInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkNodeException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException,
			ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, Node.class, httpResponse));
			break;
		}
		case 404: {
	...
	public Node unwrap() throws IOException, JsonParseException, JsonMappingException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<Node>() {
			@Override
			public Node caseOk(ResponseValue<Node> responseValue)
