...
public class SecretInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<Secret> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSecretException {
			throw new OkSecretException(responseValue.get());
		}
	...
	public SecretInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSecretException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException,
			ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, Secret.class, httpResponse));
			break;
		}
		case 404: {
	...
	public Secret unwrap() throws IOException, JsonParseException, JsonMappingException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<Secret>() {
			@Override
			public Secret caseOk(ResponseValue<Secret> responseValue)
