...
public class PluginListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Plugin>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, PluginListOkException {
			throw new PluginListOkException(responseValue.get());
		}
	...
	public PluginListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			PluginListOkException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Plugin.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Plugin> unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<List<Plugin>>() {
			@Override
			public List<Plugin> caseOk(ResponseValue<List<Plugin>> responseValue)
