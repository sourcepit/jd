...
public class SystemInfoResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<SystemInfo> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSystemInfoException {
			throw new OkSystemInfoException(responseValue.get());
		}
	...
	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSystemInfoException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, SystemInfo.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public SystemInfo unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<SystemInfo>() {
			@Override
			public SystemInfo caseOk(ResponseValue<SystemInfo> responseValue)
