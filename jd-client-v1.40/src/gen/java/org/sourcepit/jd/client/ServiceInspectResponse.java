...
public class ServiceInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<Service> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkServiceException {
			throw new OkServiceException(responseValue.get());
		}
	...
	public ServiceInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkServiceException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException,
			ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, Service.class, httpResponse));
			break;
		}
		case 404: {
	...
	public Service unwrap()
		...
		return match(new Matcher<Service>() {
			@Override
			public Service caseOk(ResponseValue<Service> responseValue)
