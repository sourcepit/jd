...
public class NodeListResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<List<Node>> responseValue)
				throws IOException, JsonParseException, JsonMappingException, NodeListOkException {
			throw new NodeListOkException(responseValue.get());
		}
	...
	public NodeListResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			NodeListOkException, InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, ArrayList.class, Node.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public List<Node> unwrap() throws IOException, JsonParseException, JsonMappingException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<List<Node>>() {
			@Override
			public List<Node> caseOk(ResponseValue<List<Node>> responseValue)
