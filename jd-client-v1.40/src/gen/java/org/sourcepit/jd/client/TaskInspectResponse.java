...
public class TaskInspectResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<Task> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkTaskException {
			throw new OkTaskException(responseValue.get());
		}
	...
	public TaskInspectResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkTaskException, NotFoundErrorResponseException, InternalServerErrorErrorResponseException,
			ServiceUnavailableErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, Task.class, httpResponse));
			break;
		}
		case 404: {
	...
	public Task unwrap() throws IOException, JsonParseException, JsonMappingException, NotFoundErrorResponseException,
			InternalServerErrorErrorResponseException, ServiceUnavailableErrorResponseException {
		return match(new Matcher<Task>() {
			@Override
			public Task caseOk(ResponseValue<Task> responseValue)
