...
public class SystemVersionResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<SystemVersionOkResponse> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSystemVersionOkResponseException {
			throw new OkSystemVersionOkResponseException(responseValue.get());
		}
	...
	public SystemVersionResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSystemVersionOkResponseException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, SystemVersionOkResponse.class, httpResponse));
			break;
		}
		case 500: {
			value = matcher
	...
	public SystemVersionOkResponse unwrap()
			throws IOException, JsonParseException, JsonMappingException, InternalServerErrorErrorResponseException {
		return match(new Matcher<SystemVersionOkResponse>() {
			@Override
			public SystemVersionOkResponse caseOk(ResponseValue<SystemVersionOkResponse> responseValue)
