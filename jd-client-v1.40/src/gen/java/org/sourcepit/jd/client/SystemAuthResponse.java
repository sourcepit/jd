...
public class SystemAuthResponse implements Closeable {
	public static interface Matcher<T> {
		default T caseOk(ResponseValue<SystemAuthOkResponse> responseValue)
				throws IOException, JsonParseException, JsonMappingException, OkSystemAuthOkResponseException {
			throw new OkSystemAuthOkResponseException(responseValue.get());
		}
	...
	public SystemAuthResponse(ObjectMapper objectMapper, HttpResponse httpResponse) {
		this.objectMapper = objectMapper;
		this.httpResponse = httpResponse;
	}

	public <T> T match(Matcher<T> matcher) throws IOException, JsonParseException, JsonMappingException,
			OkSystemAuthOkResponseException, NoContentException, InternalServerErrorErrorResponseException {
		T value;
		switch (httpResponse.getStatusLine().getStatusCode()) {
		case 200: {
			value = matcher.caseOk(new ResponseValue<>(objectMapper, SystemAuthOkResponse.class, httpResponse));
			break;
		}
		case 204: {
